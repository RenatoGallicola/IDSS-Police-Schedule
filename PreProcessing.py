...
output_file_path = 'processed_crime_data_balanced_70_negative_optimized.csv'
if os.path.exists(output_file_path):
    ...
    # Iterate through each unique combination of area, day, month, and year
    grouped = df.groupby(['area_name', 'day', 'month', 'year'])
    for (area, day, month, year), group in tqdm(grouped, desc="Generating negative data"):
        num_crimes = len(group)
        num_negatives_to_add = int(num_crimes * (70 / 30))  # Set negative cases to achieve approximately 70% negatives

        # Generate random 'fix_lat' and 'fix_lon' values directly using NumPy
        ...
        # Randomly select pairs for the required number of negatives
        if len(possible_lat_lon_pairs) < num_negatives_to_add:
            selected_pairs = possible_lat_lon_pairs  # Use all available pairs if fewer than needed
        else:
            selected_indices = np.random.choice(len(possible_lat_lon_pairs), size=num_negatives_to_add, replace=False)
            selected_pairs = possible_lat_lon_pairs[selected_indices]
    ...
    columns_to_keep = ['hour', 'minute', 'day', 'month', 'year', 'day_of_week', 'fix_lat', 'fix_lon', 'area_name',
                       'crime_occurrence']
    ...
    print("Dataset sorted in chronological order.")
...
print("Dataset processed_crime_data_balanced_70_negative_optimized loaded successfully.")
...
category_columns = ['day_of_week', 'area_name']
...
training_set.to_csv('training_set_bal_70.csv', index=False)
validation_set.to_csv('validation_set_bal_70.csv', index=False)
test_set.to_csv('test_set_bal_70.csv', index=False)

print("Datasets saved: training_set_bal_70.csv, validation_set_bal_70.csv, test_set_bal_70.csv")
